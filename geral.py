# Realiza a capitura de dados fundamentalista da lista de ações que consideramos atuais
...
j = 2  # Representa o total de ações verificadas no site fundamentus, com datas do ultimo dia de negociações e
...
for i in listStocks:
    ...
    # O else execultará o código caso a data seja encontrada
        ...
        print("O ultimo balanço foi: " , balanco)
        ...
        for coluna in range(2, len(listDados) + 2, 1):
            sheet.cell(row=j, column=coluna+1).value = listDados[coluna-2]



        listDados = [pa_n, vard_n, data, p_vpa_n, pl_n, lpa_n, vpa_n,
                     divbppa_n, p_ebit_n, div_yield_n, roe_n, varm_n, var30d_n, var12m_n, var2020_n, var2019_n,
                     var2018_n, var2017_n, var2016_n, var2015_n, balanco]
        ...
        for coluna in range(9, len(listDados) + 9, 1):
            sheet.cell(row=j, column=coluna+1).value = listDados[coluna - 9]


        # O arquivo é salvo assim que completada a condição elif
        data_e_hora_atuais = datetime.now()
        data_e_hora_em_texto = data_e_hora_atuais.strftime('%d/%m/%Y %H:%M')
        sheet.cell(row=j, column=31).value = data_e_hora_em_texto
